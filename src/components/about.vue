...
const data = reactive({
  ...
  galleryImages: [...picturesPageConfig.galleryImages],
})
...
async function fetchContent() {
  ...
  try {
    ...
    const item = json.data.find(i => i.sectionKey === 'about/content')
    if (!item) return
    const d = JSON.parse(item.contentJson)
    if (d.title != null) data.title = d.title
    if (d.contactLabel != null) data.contactLabel = d.contactLabel
    if (d.contactEmail != null) data.contactEmail = d.contactEmail
    if (d.galleryImages) d.galleryImages.forEach((url, i) => {
      if (i < 10 && url != null) data.galleryImages[i] = url
    })
  } catch { /* use defaults */ }
}
...
<template>
  <main class="pictures-page">
    ...
    <div class="overlay overlay-center">
      <h1><span>{{ data.title.slice(0, 3) }}</span>{{ data.title.slice(3) }}</h1>
    </div>

    <div class="overlay overlay-bottom">
      <p>{{ data.contactLabel }}</p>
      <p class="muted">{{ data.contactEmail }}</p>
    </div>
